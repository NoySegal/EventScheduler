...
static const std::string FILE_PATH = "testOutputs_partC";
...
bool testRegistrationAndUnRegistration() {
    ...
    try {
        s.registerToEvent(DateWrap(8, 1, 2000), "an open event", 0);
    }
    ...
    try {
        s1.registerToEvent(DateWrap(5, 1, 2000), "an open event", 0);
    }
    catch (mtm::EventDoesNotExist&) {
        cout << "EventDoesNotExist" << endl;
    }
